...
Features extracted per node:
  ...
  - nlcd_tcc_2020       : Tree canopy cover % (2020)
  - nlcd_tcc_2021       : Tree canopy cover % (2021)
  - nlcd_tcc_2022       : Tree canopy cover % (2022)
...
MODEL_CONFIGS = {
    'Model_1': {
        ...
        'suffix':      'b9dfaccb-57a8-4d1c-a78d-6b098e078f15',
    ...
    'Model_2': {
        ...
        'suffix':      '3f15a437-9803-4e1d-be03-b903b04a83e0',
...
def extract_for_model(model_name, cfg):
    ...
    lndcov_path = glob.glob(os.path.join(raster_dir, f'*LndCov*{suffix}.tiff'))
    assert lndcov_path, f"LndCov raster not found for {model_name}"
    ...
    fctImp_path = glob.glob(os.path.join(raster_dir, f'*FctImp*{suffix}.tiff'))
    assert fctImp_path, f"FctImp raster not found for {model_name}"
    ...
    # --- Tree Canopy Cover (4 years: 2020-2023) ---
    for year in [2020, 2021, 2022, 2023]:
        tcc_paths = glob.glob(os.path.join(raster_dir, f'*tcc*{year}0101*{suffix}.tiff'))
        if not tcc_paths:
            print(f"  WARNING: TCC {year} not found, filling with NaN")
            results[f'nlcd_tcc_{year}'] = np.nan
            continue
        tcc_path = tcc_paths[0]
        print(f"  Sampling TCC {year}: {os.path.basename(tcc_path)}")
        tcc = sample_raster_at_points(tcc_path, xs_r, ys_r, nodata_fill=np.nan)
        results[f'nlcd_tcc_{year}'] = tcc.astype(np.float32)
